...
class ProductController {
  ...
  async getAllProductsByUser(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      // db query 
  ...
  async getAllProducts(req: Request, res: Response, next: NextFunction) {
    try{
      ...
      let filter: Record<string, any> = { availabilityStatus: { $ne: "not available" }}
      if (req.query.search) {
        filter = {
          ...filter,
          $or: [
  ...
  async updateProduct(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const mappedData = await ProductService.mapProductForUpdateModel(req, productDetail);
      // update 
      const update = await ProductService.updateSinglRowByFilter({slug: slug}, mappedData)
      res.json({
        data: update, 
