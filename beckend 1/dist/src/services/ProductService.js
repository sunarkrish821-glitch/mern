...
class ProductService {
    ...
    static mapProductDetail(prod) {
        return {
            thumbnail: `${app_env_1.AppConfig.assetUrl}uploads/products/${prod?.thumbnail?.filename}`,
            _id: prod._id,
            title: prod.title,
            slug: prod.slug,
    ...
    static async getAllRowsByFilter(filter = {}, projection = {}, config = { page: 1, limit: 10 }) {
        try {
            // 100
            // 10 -> 10
            // page = 1, 0-9, skip = 0
    ...
    static async getSingleRowByFilter(filter) {
        try {
            const data = await ProductModel_1.default.findOne(filter).populate("category");
            return data;
        }
        catch (exception) {
            throw exception;
    ...
    static async updateSinglRowByFilter(filter, mappedData) {
        try {
